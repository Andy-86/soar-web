...
func (vEnv *VirtualEnv) BuildVirtualEnv(rEnv *database.Connector, SQLs ...string) bool {
	...
	// 置空错误信息
	vEnv.Error = nil
	//检测是否已经创建初始数据库，如果未创建则创建一个名称 hash 过的映射数据库(检查是否存在映射数据库)
	if vEnv.DBRef == nil || len(vEnv.DBRef) == 0 {
	...
	for _, sql := range SQLs {
		...
		for db, table := range meta {
			//vEnv.DBRef[rEnv.Database] = rEnv.Database
			//vEnv.Hash2DB[rEnv.Database] = rEnv.Database
			//if vEnv.TableMap[rEnv.Database] == nil {
			//	vEnv.TableMap[rEnv.Database] = make(map[string]string)
			//}
			//for _, tb := range table.Table {
			//	vEnv.TableMap[rEnv.Database][tb.TableName] = tb.TableName
			//}
			//return true
...
func (vEnv *VirtualEnv) createTable(rEnv *database.Connector, tbName string) error {
	...
	if len(vEnv.Hash2DB[vEnv.Database]) > 0 {
		vEnv.Database = vEnv.Hash2DB[rEnv.Database]
	}
