...
从所有相等的条件中选择列（WHERE COL = ..）。
以任何顺序使这些成为索引的第一列。 对于CURSOR41，三星级索引将以
LNAME，CITY或CITY，LNAME列。 在这两种情况下，必须扫描的索引片都将尽可能薄。
...
添加ORDER BY列。 不要更改这些列的顺序，而是忽略在步骤1中已经选择的列。例如，如果CURSOR41在ORDER BY中有多余的列，例如说ORDER BY LNAME，FNAME或
ORDER BY FNAME，CITY，则只有FNAME是 在此步骤中添加。 当FNAME是第三索引列时，结果表将按正确的顺序排序。 第一个FETCH调用将返回FNAME值最小的行。
...
添加SELECT语句中所有其余的列。 在此步骤中添加的列的顺序对SELECT的性能没有影响，但是应该通过在最后放置易失列来减少更新的成本。
现在，索引包含仅索引访问路径所需的所有列。
