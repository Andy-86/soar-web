...
func doSqlQuery(w http.ResponseWriter, r *http.Request) {
	...
	if len(sql) == 0 || len(databaseName) == 0 {
		fmt.Fprint(w, "parameters dose not exit")
		return
	}
	//设置当前使用的Sql
	common.Config.Query = sql
	//设置当前数据库
	fmt.Println(common.Config.Databases[databaseName])
	common.Config.TestDSN = common.Config.Databases[databaseName]
	common.Config.OnlineDSN = common.Config.Databases[databaseName]
	result := do(databaseName)
	fmt.Fprint(w, result)
}
...
func do(key string) string {
	common.Log.Debug("begin sql evaluate Query string is : %s", common.Config.Query)
	// 全局变量
	var err error
	...
	if len(buf) == 0 {
		common.Log.Debug("line 121 collapse .....")
		return ""
	}
	...
	if isContinue, _ := reportTool(buf, bom); !isContinue {
		//os.Exit(exitCode)
		common.Log.Debug("line 133 collapse .....")
		return ""
	}
	...
	for ; ; sqlCounter++ {
		...
		if syntaxErr != nil {
			...
			if common.Config.OnlySyntaxCheck || common.Config.ReportType == "rewrite" ||
				...
				//os.Exit(1)
				common.Log.Debug("line 242 collapse .....")
				return errContent
			}
		...
		if common.Config.ReportType == "rewrite" {
			if strings.HasPrefix(strings.TrimSpace(strings.ToLower(sql)), "create") ||
				...
				if rw == nil {
					...
					common.Log.Debug("line 420 collapse .....")
					//os.Exit(1)
					return ""
				}
