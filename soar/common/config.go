...
type Configuration struct {
	// +++++++++++++++测试环境+++++++++++++++++
	DatabasesNmae           string `yaml:"databasesNmae"`               // 多数据库源名
	DatabasesSource		    []*Dsn  `yaml:"databasesSource"`
	OnlineDSN               *Dsn   `yaml:"online-dsn"`                // 线上环境数据库配置
	TestDSN                 *Dsn   `yaml:"test-dsn"`                  // 测试环境数据库配置
	Databases               map[string]*Dsn                           //多个数据源
...
func readCmdFlags() error {
	...
	//多数据源配置
	databases := flag.String("databases", "", "多数据源配置，eg public，就是public-dsn 的命名 ")
	...
	for _,dsnConfig :=range Config.DatabasesSource {
		ortherDsn :=  FormatDSN(dsnConfig)
		Config.Databases[dsnConfig.Alias] = ParseDSN(ortherDsn, Config.Databases[dsnConfig.Alias])
	}
