...
func IsColsPart(a, b []*Column) bool {
	...
	for ; i < times; i++ {
		j := 0
		for ; j < len(b); j++ {
			if strings.ToLower(a[i].DB) == strings.ToLower(b[j].DB) &&
				strings.ToLower(a[i].Table) == strings.ToLower(b[j].Table) &&
				strings.ToLower(a[i].Name) == strings.ToLower(b[j].Name) {
				break
			}
		}
		if j == len(b) {
	...
	// innodb使用最左前匹配，最左一个都没有匹配上的话就认为是每匹配上索引
	if i == 0 {
		return false
	}
